...
// [[Rcpp::export]]
arma::mat K(const arma::vec& sigma, const arma::mat& K1, const arma::mat& X) {
  // Calculate the exponential of each element in sigma
  arma::vec exp_sigma = arma::exp(sigma);
  
  // Create the combined matrix K1
  arma::mat combined_K = K1;
  
  // Scale the combined matrix by each element of exp_sigma
  // Assuming that the first element of exp_sigma is used for scaling K1 + K2
  combined_K *= exp_sigma(0);
  
  // Create the diagonal matrix with 0.1 on the diagonal
  arma::mat diag_mat = arma::diagmat(arma::vec(X.n_cols, arma::fill::ones) * 0.0001);
  
  // Construct the block diagonal matrix
  arma::mat result = arma::join_cols(
    arma::join_rows(combined_K, arma::zeros(combined_K.n_rows, diag_mat.n_cols)),
    arma::join_rows(arma::zeros(diag_mat.n_rows, combined_K.n_cols), diag_mat)
  );
...
// [[Rcpp::export]]
double calc_ljoint(const arma::mat& A, const arma::vec& beta, const arma::vec& sigma, const arma::mat& Al, const arma::mat& K1, const arma::mat& X, const arma::uvec& index) {
  // Calculate gamma
  arma::vec gamma = gamma_f(beta, index);
  
  // Compute Cholesky decomposition of K(sigma, K1, K2)
  arma::mat chol_Q = arma::chol(K(sigma, K1,X));
...
arma::vec calc_grad_ff(const arma::vec& beta, const arma::vec& sigma,
                       ...
                       const arma::uvec& index, const arma::mat& K1, const arma::mat& X) {
  // Calculate gamma
  arma::vec gamma = gamma_f(beta, index);
  ...
  arma::vec term3 = K(sigma, K1,X) * beta;
...
// [[Rcpp::export]]
arma::mat calc_neg_hess_ff4(const arma::vec& beta, const arma::vec& sigma, const arma::mat& A, const arma::mat& Al, const arma::mat& K1, const arma::mat& X, const arma::uvec& index) {
  ...
  return -(Hess - K(sigma, K1,X));
}

// [[Rcpp::export]]
arma::vec calc_x0(const arma::vec& alpha, arma::vec x0, const arma::mat& A, const arma::mat& Al,
                  const arma::uvec& index, const arma::mat& K1,  const arma::mat& X, double tol = 1e-12) {
  arma::vec x = x0;
  int cont = 0;
  const int max_iterations = 50;
  
  while (true) {
    arma::vec g1 = calc_grad_ff(x, alpha,A,Al,index,K1,X);
    cont += 1;
    arma::mat H = -calc_neg_hess_ff4(x, alpha,A,Al,K1,X, index);
...
double calc_lpost_cpp(const arma::vec& sigma, const arma::vec& xin,
                      const arma::mat& A, const arma::mat& Al,
                      const arma::uvec& index, const arma::mat& K1, const arma::mat& X, const size_t n) {
  // Calculate x0 using the provided function
  arma::vec x0 = calc_x0(sigma, xin, A, Al, index, K1,X);

  // Calculate H using the provided function (H is a sparse matrix)
  arma::mat H = calc_neg_hess_ff4(xin, sigma, A, Al, K1, X,index);
  ...
  double log_joint = calc_ljoint(A, x0, sigma, Al, K1, X,index);
...
double calc_lpost3(const arma::vec& sigma, const arma::vec& xin, const arma::mat& A, 
                   const arma::mat& Al, const arma::uvec& index, const arma::mat& K1, const arma::mat& X, size_t n) {
  // Calculate x0 using the provided function
  arma::vec x0 = calc_x0(sigma, xin, A, Al, index, K1,X);
