...
arma::sp_mat K(const arma::vec& sigma, const arma::mat& K1, const arma::mat& K2, const arma::mat& K3, const arma::mat& X) {
  ...
  arma::mat part1 = exp(sigma[0]) * K1;
  
 
  arma::mat part2 = exp(sigma[0]) * K2;
  
  
  arma::mat part3 = exp(sigma[1]) * K3;
  
 
  arma::sp_mat diag_mat = arma::speye<arma::sp_mat>(X.n_cols, X.n_cols) * 0.01;
  
  // Determine the size of the resulting square matrix
  int total_cols = part1.n_cols + part2.n_cols+ part3.n_cols + diag_mat.n_cols;
  
  
  // Initialize the sparse result matrix
  arma::sp_mat result(total_cols, total_cols);

  // Fill the blocks in the diagonal
  int current_col = 0;
  
  // Place part1
  result.submat(current_col, current_col, current_col + part1.n_rows - 1, current_col + part1.n_cols - 1) = arma::sp_mat(part1);
  current_col += part1.n_cols;
  
  // Place part2
  result.submat(current_col, current_col, current_col + part2.n_rows - 1, current_col + part2.n_cols - 1) = arma::sp_mat(part2);
  current_col += part2.n_cols;
  
  // Place part2
  result.submat(current_col, current_col, current_col + part3.n_rows - 1, current_col + part3.n_cols - 1) = arma::sp_mat(part3);
