...
plot(datapred$qm,type = "l", col =  "darkblue",lwd = 2,lty =2, cex.axis = 1.5,cex.lab = 1.5, ylab = "Cholesterol level")
...
TT = (A2d%*%t(gamma)[,(dim(A1)[2]+1):(dim(A1)[2]+ 27+ 27*4)])
plot((((-TT[data$idf == 1])[seq(1,64, 8)])[1]/(A1d%*%t(gamma)[,1:9])))

((-TT[data$idf == 1])[seq(1,64, 8)])[1]/mean((A1d%*%t(gamma)[,1:9]))
...
print(ggplot(data = data_1, aes(x = time, y = VY,group = id,colour = id))+
        ...
        theme_bw() +theme(axis.line = element_line(colour = "black"),
                          ...
                          panel.background = element_blank())+ theme(axis.text=element_text(size=16),
                                                                     axis.title=element_text(size=16,face="bold"), strip.text = element_text(
                                                                       size = 16),legend.text = element_text(size = 14),legend.title = element_text(size = 16)))
