...
sspec <- s(Reaction, k = 10 , bs = "ps")
sspec$mono <- 1
sm1 <- smoothCon(sspec, data = sleepstudy, scale.penalty=FALSE, absorb.cons = TRUE)[[1]]
A <- sm1$X
q = dim(A)[2]
# Getting the derivative of h1(y)
sm1$deriv <- 1
Ad <- PredictMat(sm1, sleepstudy)
index1 = sm1$g.index
...
b0 = apply(beta,2,mean)[12:(12+17)]
b1 = apply(beta,2,mean)[(13+17):ncol(A)]
...
head(datanew)
Ay <- PredictMat(sm1, datanew)
sm1$deriv = 1
Ayd <- PredictMat(sm1, datanew)
...
days_effects = (Xm[,c(1:2)]%*%apply(gamma2,1,mean)[c(10:11)])
...
marginal_days = function(i){  
  ...
  h1 = apply(sweep(Ay%*%gamma2[1:ncol(Ay),], 2, c(apply(days_effects[i] + X2%*%(gamma2[12:ncol(A),]),2,mean)) , "+"),1,mean)
  fy = dnorm(h1)*h1l
  return(fy)
}
...
#######################################################################
median_impact308 = apply((-(beta[,11] + beta[,30])/t(Ayq%*%gamma2[1:ncol(Ayq),])),2,mean)
median_impact309 = apply((-(beta[,11] + beta[,31])/t(Ayq%*%gamma2[1:ncol(Ayq),])),2,mean)
